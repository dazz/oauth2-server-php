...
class OAuth2_Server_Access_BasicValidationTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testMalformedHeader()
    {
        $server = $this->getTestServer();
        $request = OAuth2_Request::createFromGlobals();
        $request->server['HTTP_AUTHORIZATION'] = 'tH1s i5 B0gU5';
        $allow = $server->verifyAccessRequest($request);
        $this->assertFalse($allow);
    ...
    public function testInvalidToken()
    {
        $server = $this->getTestServer();
        $request = OAuth2_Request::createFromGlobals();
        $request->server['HTTP_AUTHORIZATION'] = 'Bearer TESTTOKEN';
        $allow = $server->verifyAccessRequest($request);
        $this->assertFalse($allow);
    ...
    public function testExpiredToken()
    {
        $server = $this->getTestServer();
        $request = OAuth2_Request::createFromGlobals();
        $request->server['HTTP_AUTHORIZATION'] = 'Bearer accesstoken-expired';
        $allow = $server->verifyAccessRequest($request);
        $this->assertFalse($allow);
    ...
    public function testOutOfScopeToken()
    {
        $server = $this->getTestServer();
        $request = OAuth2_Request::createFromGlobals();
        $request->server['HTTP_AUTHORIZATION'] = 'Bearer accesstoken-scope';
    ...
    public function testMalformedToken()
    {
        $server = $this->getTestServer();
        $request = OAuth2_Request::createFromGlobals();
        $request->server['HTTP_AUTHORIZATION'] = 'Bearer accesstoken-malformed';
        $allow = $server->verifyAccessRequest($request);
        $this->assertFalse($allow);
